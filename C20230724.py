...
for gen in range(iteration):
    ...
    for i in range(num_populations):
        ...
        #print('ranked_pop',ranked_pop)
        # 对 ranked_pop进行交叉
        for ii in range(1, len(ranked_pop), 2):
            if random.random() < cxpb:
                ranked_pop[ii-1],ranked_pop[ii] = cxSimulatedBinaryBounded(ranked_pop[ii-1], ranked_pop[ii], eta, low, up)
        
        mating_pool = ranked_pop
