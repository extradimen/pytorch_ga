...
for p, s in zip(P, SCALES):
    ref_points.append(torch.from_numpy(np.random.uniform(size=(p, problem.n_obj))) * s)
...
for gen in range(iteration):
        ...
        if random.random() < MUTPB:
            ...
            for j in range(problem.n_var):
                if random.random() < 1.0 / problem.n_var:
                    lower = max(problem.xl, mutant[j] - 0.1)
                    upper = min(problem.xu, mutant[j] + 0.1)
    ...
    print(calculate_igd(fitness_values, pf))
