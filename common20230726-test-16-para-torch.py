...
for gen in range(iteration):
    ...
    if __name__ == '__main__':
        ...
        results_ = pool.map(para_evolve_1.para_evolve1,populations)
    ...
    # 处理每个种群的演化结果
    for i, (population, igd_) in enumerate(results_):
        populations[i] = population  # 更新每个种群的演化结果
        # 处理igd值，这里可以根据需要保存或使用igd值
        igd_all.append(igd_)
    # insert finish
